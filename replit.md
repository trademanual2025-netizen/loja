...
- **Coupon/Discount System**: Prisma `Coupon` model with admin CRUD (`/api/admin/coupons`), public validate (`/api/coupons/validate`) and active-check (`/api/coupons/active`) APIs. Supports percentage/fixed discounts with scoping (all/categories/products). Checkout UI shows coupon input when active coupons exist. Server-side coupon re-validation in both MP and Stripe checkout routes (never trusts client discount values). `usedCount` incremented on PAID status (at checkout for instant approvals, in webhooks for async payments) with idempotency guards.
...
- **Admin Roles & Permissions**: Two roles: `superadmin` (full access, can manage other admins' permissions) and `admin` (access controlled by permissions array). 17 modules: dashboard, products, categories, orders, cupons, reembolsos, leads, comunicacao, mensagens, admins, integracoes, settings, tracking, webhooks, frete, embed, perfil. Tracking, Webhooks, and Frete are standalone sidebar modules (extracted from Settings tabs) with independent permission control. Settings API auto-detects the module from the keys being saved and enforces the appropriate permission. Sidebar filters nav items by permissions. API routes enforce permissions server-side via `requirePermission()`. AdminGuard component redirects unauthorized page access. Superadmin: `leogracio42@gmail.com`, Admin: `gcdias97@gmail.com`.
...
- **Admin Settings Pipeline**: Settings are saved via POST `/api/admin/settings` → stored in `Settings` table → cache invalidated via `revalidateTag('settings')` + `clearSettingsCache()` → pages revalidated via `revalidatePath`. Theme-independent overrides (--primary, --btn-buy, --btn-header) are applied as inline styles on `<html>`. Theme-dependent overrides (--text, --bg, --bg-card, --text-title, --icon-cart) are scoped to `[data-theme='dark']` via a `<style>` tag to avoid overriding light mode values.
